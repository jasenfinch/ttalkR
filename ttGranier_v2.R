...
ttGranier_v2 <- function (mydata_4D, plot_label)
  ...
  mydata_4D$dTon <- Theat_1C - Tref_1C # end of heating
  mydata_4D$dToff <- Theat_0C - Tref_0C # before heating
...
#  }
  ...
  for (j in 1:(length(ID))) {
    df <- data.frame(
      ...
      Date = mydata_4D$SDate[mydata_4D$TT_ID == ID[j]],
      Time = mydata_4D$STime[mydata_4D$TT_ID == ID[j]]
    )
    colnames(df) <- c("dTon", "dToff", "Date", "Time")
    ...
    dTo <- aggregate(dTon[Time < "03:00:00"] - dToff[Time < "03:00:00"] ~ Date[Time < "03:00:00"], df, max, simplify = FALSE, na.action = na.omit)
    colnames(dTo) <- c("Date", "dTo")
  ...
  mydata_4D$dTo[mydata_4D$dTo == "Null"] <- NA
  dTo <- as.numeric(unlist(mydata_4D$dTo)) #under no flux conditions (nighttime?)
  dTu <- as.numeric(unlist(mydata_4D$dTon - mydata_4D$dToff)) #for every half an hour
  #
  # Calculate K
  #
  ...
  if (plot_label == "split") {
    p <- ggplot(data = df1) +
      ...
      ylim(0, max(c(Fd, Fd_Do), na.rm = TRUE))
  ...
  colnames(df_ttGranier) <- c("Timestamp", "Fd", "Fd_Do", "TT_ID")# # # # # # #  NEW LINE
