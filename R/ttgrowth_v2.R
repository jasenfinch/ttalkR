...
ttGrowth <- function(mydata_4D, plot_label) { #this is a beta function
  ...
  for (j in 1:length(ID)) {
    ...
    myDendro_data_daily <- spline(myDendro_data_weekly$dendrometer, n=(length(myDendro_data_L1$dendrometer)))
    ...
    #myDendro_data_L2$dendrometer <- myDendro_data_daily$y
  ...
  if (plot_label == "split"){
    p <- ggplot(data = df1, aes(Timestamp, dendrometer, color = id_col_ind)) +
      ...
      #labs(x = "Timestamp", y = "radial growth (mm)") +
      labs(x = element_blank(), y = "radial growth (mm)") +
      scale_color_gradientn(colours = hcl.colors(30, palette = "viridis")) +
      scale_x_datetime(minor_breaks = ("1 week")) +
      theme(legend.position = "none") +
